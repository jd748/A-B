...
int main()
{
    ...
    for (int asdf = 0; asdf < N2; asdf++){
        ...
        for (int i = 0; i < n; i++){
            ...
            try{
                plus = (1/delta)*(distup_plus*table.at(n-i-1,var_delta+1, rounddown_plus) + distdown_plus*table.at(n-i-1,var_delta+1,roundup_plus));
                minus = (1/delta)*(distup_minus*table.at(n-i-1,var_delta-1, rounddown_minus) + distdown_minus*table.at(n-i-1,var_delta-1,roundup_minus));
                if (minus >= plus){
                    var_delta++;
    ...
    outputFile.open("Random50_45.txt");

    for (int l = 0; l < n; l++){
        for (int i=0; i < 2*n+1;i++){
            outputFile << table.at(l,i,0) << " ";
        }
        //outputFile << *std::cout << std::endl;
        outputFile << "\n";
    } 
